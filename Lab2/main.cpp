...
int main()
{
    cout<<"\nIn this we don't consider blank tile and see obeying of Monotonic Restriction.\n";
    Node *S = new Node();
    int G[3][3];
    ...
    if (results[0].success || results[1].success || results[2].success || results[3].success)
    {
        ...
        {
            if (verifySubset(results[1].CL, results[0].CL))
            {
                if (verifySubset(results[0].CL, results[3].CL))
                {
